...
func csb2csd(inFile, outFile string) {
    ...
    content := csdparser.CreateFromCSB(parser)
    ...
    propertyGroup.CreateAttr("ID", uuid.Must(uuid.NewV4()).String())
...
func csb2json(inFile, outFile string) {
    ...
    parser := csbparser.GetRootAsCSParseBinary(data, 0)
    jData := (&CSParseBinary{parser.Table()}).GetData()
...
func json2csb(inFile, outFile string) {
    ...
    builder.Finish(CreateCSParseBinary(builder, jData))
